...
RUN apt-get update && apt-get install -y --no-install-recommends \
    ...
    libcups2-dev libavahi-client-dev libusb-1.0-0-dev \
    libpng-dev zlib1g-dev libssl-dev \
 && rm -rf /var/lib/apt/lists/*

# Build PAPPL (required by LPrint)
...
RUN git clone --depth 1 https://github.com/michaelrsweet/lprint.git /src/lprint \
 && cd /src/lprint \
 && ./configure --prefix=/usr --with-dnssd=avahi --enable-libpng --enable-libusb \
 && make -j"$(nproc)" \
 && make install
...
RUN apt-get update && apt-get install -y --no-install-recommends \
    libcups2 libavahi-client3 libusb-1.0-0 libpng16-16 zlib1g ca-certificates \
 && rm -rf /var/lib/apt/lists/*
